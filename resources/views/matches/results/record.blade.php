...
<div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-gradient-to-br from-[#2a2a2a] to-[#252525] rounded-xl p-8 border border-[#3a3a3a] shadow-xl">
        ...
        <form method="POST" action="{{ route('matches.results.store', $match) }}" id="resultsForm">
            ...
            <div class="mb-8">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-400">Agrega los goles anotados (opcional, minuto opcional)</p>
                    </div>
            ...
            <!-- Opponent Participants Section -->
            <div class="mb-8">
                <h3 class="text-xl font-bold text-[#CDFDE6] mb-4">Participantes de {{ $opponentTeam->name }}</h3>
                <p class="text-sm text-gray-400 mb-4">Selecciona los jugadores del equipo contrario que participaron:</p>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    @foreach($opponentTeam->members as $member)
                    <label class="flex items-center p-3 bg-[#3a3a3a] rounded-lg cursor-pointer hover:bg-[#4a4a4a] transition-colors border border-[#4a4a4a]">
                        <input type="checkbox" name="opponent_participants[]" value="{{ $member->id }}" class="opponent-participant-checkbox w-4 h-4 text-[#01FF87] bg-gray-700 border-gray-600 rounded focus:ring-[#01FF87] focus:ring-2">
                        <span class="ml-3 text-[#CDFDE6]">{{ $member->name }}</span>
                    </label>
                    @endforeach
                </div>

                @error('opponent_participants')
                <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>

            <!-- Opponent Goals Section -->
            <div class="mb-8">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        <h3 class="text-xl font-bold text-[#CDFDE6]">Goles de {{ $opponentTeam->name }}</h3>
                        <p class="text-sm text-gray-400">Agrega los goles del equipo contrario (opcional, minuto opcional)</p>
                    </div>
                    <button type="button" onclick="addOpponentGoal()" class="px-4 py-2 bg-[#01FF87] text-[#1f1f1f] font-bold rounded-lg hover:shadow-lg hover:shadow-[#01FF87]/20 transition-all duration-200">
                        + Agregar Gol
                    </button>
                </div>

                <div id="opponentGoalsContainer" class="space-y-3">
                    <!-- Opponent goals will be added here dynamically -->
                </div>

                <div id="noOpponentGoalsMessage" class="text-center py-8 text-gray-500">
                    <p>No se han agregado goles. Haz clic en "Agregar Gol" para registrar un gol.</p>
                </div>
            </div>
            ...
            <div class="bg-[#3a3a3a] rounded-lg p-6 mb-8 border border-[#4a4a4a]">
                ...
                <div class="flex items-center justify-center gap-6">
                    ...
                    <div class="text-center">
                        ...
                        <p id="opponentTeamScore" class="text-4xl font-bold text-[#01FF87]">0</p>
                    </div>
                </div>
                <p class="text-xs text-center text-gray-500 mt-3">Registra los resultados completos del partido</p>
            </div>
...
<script>
    ...
    function getSelectedOpponentParticipants() {
        const checkboxes = document.querySelectorAll('.opponent-participant-checkbox:checked');
        return Array.from(checkboxes).map(cb => ({
            id: cb.value,
            name: cb.nextElementSibling.textContent.trim()
        }));
    ...
    function addGoal() {
        ...
        <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3">
            <select name="goals[${goalCount}][scorer_id]" class="px-3 py-2 bg-[#2a2a2a] border border-[#4a4a4a] rounded-lg text-[#CDFDE6] focus:outline-none focus:ring-2 focus:ring-[#01FF87]">
                <option value="">Desconocido</option>
                ${participants.map(p => `<option value="${p.id}">${p.name}</option>`).join('')}
            </select>
            <input type="number" name="goals[${goalCount}][minute]" min="1" max="120" placeholder="Minuto (opcional)" class="px-3 py-2 bg-[#2a2a2a] border border-[#4a4a4a] rounded-lg text-[#CDFDE6] placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#01FF87]">
        </div>
    ...
    function addOpponentGoal() {
        const participants = getSelectedOpponentParticipants();

        if (participants.length === 0) {
            alert('Primero debes seleccionar al menos un participante del equipo contrario.');
            return;
        }

        opponentGoalCount++;
        const container = document.getElementById('opponentGoalsContainer');
        const noGoalsMessage = document.getElementById('noOpponentGoalsMessage');

        noGoalsMessage.style.display = 'none';

        const goalDiv = document.createElement('div');
        goalDiv.className = 'flex items-center gap-3 p-4 bg-[#3a3a3a] rounded-lg border border-[#4a4a4a]';
        goalDiv.id = `opponent-goal-${opponentGoalCount}`;
        ...
        <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3">
            <select name="opponent_goals[${opponentGoalCount}][scorer_id]" class="px-3 py-2 bg-[#2a2a2a] border border-[#4a4a4a] rounded-lg text-[#CDFDE6] focus:outline-none focus:ring-2 focus:ring-[#01FF87]">
                <option value="">Desconocido</option>
                ${participants.map(p => `<option value="${p.id}">${p.name}</option>`).join('')}
            </select>
            <input type="number" name="opponent_goals[${opponentGoalCount}][minute]" min="1" max="120" placeholder="Minuto (opcional)" class="px-3 py-2 bg-[#2a2a2a] border border-[#4a4a4a] rounded-lg text-[#CDFDE6] placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#01FF87]">
        </div>
        <button type="button" onclick="removeOpponentGoal(${opponentGoalCount})" class="flex-shrink-0 p-2 text-red-400 hover:bg-red-500/20 rounded-lg transition-colors">
            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
            </svg>
        </button>
    ...
    function removeOpponentGoal(id) {
        const goalDiv = document.getElementById(`opponent-goal-${id}`);
        if (goalDiv) {
            goalDiv.remove();
            updateScore();
    ...
    function updateScore() {
        ...
        const opponentGoalsContainer = document.getElementById('opponentGoalsContainer');
        const userGoalCount = goalsContainer.children.length;
        const opponentGoalCount = opponentGoalsContainer.children.length;
        document.getElementById('userTeamScore').textContent = userGoalCount;
        document.getElementById('opponentTeamScore').textContent = opponentGoalCount;
    }
    ...
    document.addEventListener('change', function(e) {
        ...
        if (e.target.classList.contains('opponent-participant-checkbox')) {
            updateOpponentParticipantDropdowns();
        }
    });
    ...
    function updateParticipantDropdowns() {
        ...
        selects.forEach(select => {
            ...
            select.innerHTML = '<option value="">Desconocido</option>' +
                participants.map(p => `<option value="${p.id}" ${p.id == currentValue ? 'selected' : ''}>${p.name}</option>`).join('');
        });
    }

    function updateOpponentParticipantDropdowns() {
        const participants = getSelectedOpponentParticipants();
        const selects = document.querySelectorAll('select[name^="opponent_goals"]');
