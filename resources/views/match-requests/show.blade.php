        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-gradient-to-br from-[#2a2a2a] to-[#252525] rounded-xl p-6 border border-[#3a3a3a] shadow-xl">
                <div class="flex items-start justify-between mb-4">
                    <div>
                        <h2 class="text-2xl font-bold text-[#CDFDE6] mb-2">{{ $matchRequest->team->name }}</h2>
                        <p class="text-sm text-gray-400">Publicado {{ $matchRequest->created_at->diffForHumans() }}</p>
                    </div>
                    <span class="px-4 py-2 bg-[#01FF87]/20 text-[#01FF87] text-sm font-bold rounded-full border border-[#01FF87]/30">
                        {{ ucfirst($matchRequest->status) }}
                    </span>
                </div>

                <div class="space-y-4">
                    <div class="flex items-center gap-3 p-3 bg-[#3a3a3a] rounded-lg">
                        ...
                        <div>
                            ...
                            <p class="text-[#CDFDE6] font-semibold">{{ $matchRequest->match_datetime->format('l, d/m/Y H:i') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3 p-3 bg-[#3a3a3a] rounded-lg">
                        ...
                        <div>
                            ...
                            <p class="text-[#CDFDE6] font-semibold">{{ $matchRequest->location }}</p>
                        </div>
                    </div>

                    <div class="flex items-center gap-3 p-3 bg-[#3a3a3a] rounded-lg">
                        <span class="text-2xl">{{ $matchRequest->team->getFootballTypeIcon() }}</span>
                        <div>
                            <p class="text-xs text-gray-400">Tipo de Fútbol</p>
                            <p class="text-[#CDFDE6] font-semibold">{{ $matchRequest->team->getFootballTypeName() }}</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-gradient-to-br from-[#2a2a2a] to-[#252525] rounded-xl p-6 border border-[#3a3a3a] shadow-xl">
                ...
                <div class="space-y-4">
                    @foreach($matchRequest->applications as $application)
                    <div class="bg-[#3a3a3a] rounded-lg p-4 border border-[#4a4a4a]">
                        <div class="flex items-start justify-between mb-3">
                            <div>
                                <h4 class="text-lg font-bold text-[#CDFDE6]">{{ $application->applicantTeam->name }}</h4>
                        ...
                        <div class="flex gap-2">
                            <form method="POST" action="{{ route('match-requests.applications.accept', [$matchRequest, $application]) }}" class="flex-1">
                                @csrf
                                <button type="submit" class="w-full px-4 py-2 bg-[#01FF87] text-[#1f1f1f] font-bold rounded-lg hover:shadow-lg hover:shadow-[#01FF87]/20 transition-all duration-200">
                                    Aceptar
                                </button>
                            </form>
                        </div>
            ...
            <div class="bg-gradient-to-br from-[#2a2a2a] to-[#252525] rounded-xl p-6 border border-[#3a3a3a] shadow-xl">
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <p class="text-[#CDFDE6] font-semibold">{{ $matchRequest->creator->name }}</p>
                    </div>
                </div>
                <a href="{{ route('teams.show', $matchRequest->team) }}" class="mt-4 block text-center px-4 py-2 bg-[#3a3a3a] text-[#CDFDE6] rounded-lg hover:bg-[#4a4a4a] font-medium transition-all duration-200">
                    Ver Equipo
                </a>
            </div>
                ...
                <form method="POST" action="{{ route('match-requests.apply', $matchRequest) }}">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea id="message" name="message" rows="3" maxlength="500" class="w-full px-3 py-2 bg-[#3a3a3a] border border-[#4a4a4a] rounded-lg text-[#CDFDE6] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#01FF87]" placeholder="Mensaje para el equipo..."></textarea>
                        </div>
                        <button type="submit" class="w-full px-4 py-3 bg-gradient-to-r from-[#01FF87] to-[#00e676] text-[#1f1f1f] font-bold rounded-xl hover:shadow-lg hover:shadow-[#01FF87]/20 transition-all duration-200">
                            Aplicar
                        </button>
                    </div>
                </form>
