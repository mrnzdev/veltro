...
<div class="max-w-2xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-[#2a2a2a] rounded-xl p-8 border border-[#3a3a3a]">
        <form method="POST" action="{{ route('teams.update', $team) }}">
            ...
            <!-- Football Type -->
            <div class="mb-6">
                <label for="football_type" class="block text-sm font-medium text-[#CDFDE6] mb-3">
                    Tipo de Fútbol *
                </label>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    @foreach(\App\Models\Team::FOOTBALL_TYPES as $type => $config)
                    <label class="football-type-card cursor-pointer">
                        <input type="radio"
                            name="football_type"
                            value="{{ $type }}"
                            class="peer sr-only"
                            {{ old('football_type', $team->football_type) === $type ? 'checked' : '' }}
                            required>
                        <div class="h-full p-6 bg-[#3a3a3a] border-2 border-[#4a4a4a] rounded-xl transition-all duration-200 peer-checked:border-[#01FF87] peer-checked:bg-[#01FF87]/10 hover:border-[#01FF87]/50">
                            <div class="text-center">
                                <div class="text-4xl mb-3">{{ $config['icon'] }}</div>
                                <h4 class="text-lg font-bold text-[#CDFDE6] mb-2">{{ $config['name'] }}</h4>
                                <p class="text-sm text-gray-400">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full bg-[#2a2a2a] text-[#01FF87] font-medium">
                                        <svg class="h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path>
                                        </svg>
                                        {{ $config['max_members'] }} jugadores
                                    </span>
                                </p>
                            </div>
                        </div>
                ...
                <p class="mt-2 text-xs text-gray-400">
                    ...
                    Cambiar el tipo de fútbol actualizará automáticamente el máximo de miembros.
                </p>
            </div>
