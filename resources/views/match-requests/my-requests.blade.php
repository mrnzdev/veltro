        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
            ...
            <div class="mt-4 lg:mt-0 flex flex-col sm:flex-row gap-3">
                <a href="{{ route('match-requests.my-applications') }}"
                    class="inline-flex items-center justify-center px-5 py-2.5 bg-[#2a2a2a] border border-[#3a3a3a] text-[#CDFDE6] font-semibold rounded-xl hover:bg-[#3a3a3a] hover:border-[#01FF87]/50 transition-all duration-200">
                    Mis Aplicaciones
                </a>
                @can('create', \App\Models\MatchRequest::class)
                <a href="{{ route('match-requests.create') }}"
                    class="inline-flex items-center justify-center px-5 py-2.5 bg-gradient-to-r from-[#01FF87] to-[#00e676] text-[#1f1f1f] font-semibold rounded-xl hover:shadow-lg hover:shadow-[#01FF87]/20 hover:scale-105 transition-all duration-200">
                    <svg class="h-5 w-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="group bg-gradient-to-br from-[#2a2a2a] to-[#252525] rounded-xl p-6 border border-[#3a3a3a] hover:border-[#01FF87]/50 hover:shadow-xl hover:shadow-[#01FF87]/10 transition-all duration-300 hover:-translate-y-1 flex flex-col">
            ...
            <div class="flex items-start justify-between mb-4">
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-gray-500">Creado {{ $matchRequest->created_at->diffForHumans() }}</p>
                </div>
            ...
            <div class="space-y-3 mb-4 flex-grow">
                ...
                <div class="inline-flex items-center gap-2 px-3 py-1.5 bg-[#3a3a3a] rounded-lg text-sm">
                    ...
                    <span class="text-gray-300 font-medium">{{ $matchRequest->applications->count() }} {{ $matchRequest->applications->count() === 1 ? 'aplicación' : 'aplicaciones' }}</span>
                </div>
                @elseif($matchRequest->status === 'matched' && $matchRequest->match)
                <div class="bg-green-500/10 border border-green-500/30 rounded-lg p-3">
                    <p class="text-green-400 text-xs font-medium">
                        Partido vs <span class="font-bold">{{ $matchRequest->match->opponentTeam->name }}</span>
                    </p>
                </div>
                @endif
            </div>
